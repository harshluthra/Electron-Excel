...
$(document).ready(function() {
    ...
    $('#italic').on('click', function() {
        ...
        $('#grid .cell.selected').each(function() {
            $(this).css('font-style', bold ? 'italic' : 'normal ');
            let rid = parseInt($(this).attr('rid'));
            let cid = parseInt($(this).attr('cid'));
            let cobj = rows[rid][cid];
            cobj.italic = italic;
    ...
    $('#underline').on('click', function() {
        ...
        $('#grid .cell.selected').each(function() {
            $(this).css('text-decoration', underline ? 'underline' : 'normal ');
            let rid = parseInt($(this).attr('rid'));
            let cid = parseInt($(this).attr('cid'));
            let cobj = rows[rid][cid];
    ...
    //idhar type krne ka mtlb h ki value se formula type hua h yaaa formula update hora h
    // things to do ->
    //1- apni upstream ko update krna 
    //2- unki downstream me apna naam dena h
    //3- evaluate that formula 
    //4- invoke downstream about your updated value so that they will change there's too
